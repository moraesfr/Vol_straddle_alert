...
cat("=== PETR4 STRADDLE STRATEGY SIMULATION (V2) ===\n\n")
...
for (i in 1:nrow(alerts)) {
  ...
  # Find exit points
  exits_10 <- position_data %>% filter(pnl_pct >= 10) %>% slice(1)
  exits_15 <- position_data %>% filter(pnl_pct >= 15) %>% slice(1)
  exits_20 <- position_data %>% filter(pnl_pct >= 20) %>% slice(1)
  ...
  if (nrow(exits_20) > 0) {
    exit_date <- exits_20$date[1]
    exit_price <- exits_20$straddle_value[1]
    exit_pnl <- exits_20$pnl[1]
    exit_pnl_pct <- exits_20$pnl_pct[1]
    exit_reason <- "20% target"
  } else if (nrow(exits_15) > 0) {
    exit_date <- exits_15$date[1]
    exit_price <- exits_15$straddle_value[1]
    exit_pnl <- exits_15$pnl[1]
    exit_pnl_pct <- exits_15$pnl_pct[1]
    exit_reason <- "15% target"
  } else if (nrow(exits_10) > 0) {
    exit_date <- exits_10$date[1]
    exit_price <- exits_10$straddle_value[1]
    exit_pnl <- exits_10$pnl[1]
    exit_pnl_pct <- exits_10$pnl_pct[1]
...
# Save raw results
write_csv(straddle_results, "petr4_straddle_results.csv")
...
write_csv(straddle_results, "petr4_straddle_results_scaled.csv")
...
cat("✓ Saved: petr4_straddle_results_scaled.csv\n")
